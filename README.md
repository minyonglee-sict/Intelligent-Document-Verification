...
**2. 근거 대조** (`validator.grounding_check`, source=`rule`) — 추출된 값이 원문에
실제로 있는지 확인합니다. **환각을 결정적으로 잡습니다.** 날짜(`2021-05-04` ↔
`04.05.2021`)와 숫자(`4,936.71` ↔ `4936.71`)는 표기 차이를 펼쳐 대조하므로
정규화된 값도 오탐이 나지 않습니다. 실측: 문서 4건 오탐 0건, 심어놓은 환각 4종 전부 검출.

**3. 빈 필드 재확인** (`validator.probe_missing_fields`, source=`llm`) — 비어 있는
필드만 골라 "이게 문서에 있나?"를 되묻고, **모델이 답한 값이 원문에 실제로 있을 때만**
채택합니다. 근거가 확인되지 않는 주장은 조용히 버립니다. 빈 필드가 없으면 LLM 호출
자체를 건너뜁니다.
...
지금은 Markdown 표를 파이썬으로 직접 읽습니다. 실측 결과 **문서 4건 140행 전부
`수량 x 단가 = 금액` 검산 통과**, 0건이던 93행 문서가 84행 전부 추출됩니다.
...
| **근거 대조** | 추출값이 원문에 있는지 (환각 차단) | **0.01초 미만** |
